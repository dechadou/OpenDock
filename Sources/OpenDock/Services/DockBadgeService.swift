...
final class DockBadgeService: ObservableObject {
    ...
    private static let debugBadgesEnabled = ProcessInfo.processInfo.environment["OPENDOCK_DEBUG_BADGES"] == "1"
    private let badgeLogger = Logger(subsystem: AppIdentity.bundleIdentifier, category: "badges")
...
public enum DockBadgeParser {
    public static func extractBadgeText(for appName: String, from groups: [[String]]) -> String? {
        let normalizedAppName = normalizedForMatching(appName)
        ...
        let matchingGroup = groups.first { group in
            group.contains { text in
                normalizedForMatching(text).contains(normalizedAppName)
            }
        }

        return matchingGroup.flatMap { extractBadgeText(from: $0, appName: appName) }
    }
    ...
    public static func extractBadgeText(from texts: [String], appName: String) -> String? {
        ...
        for text in trimmedTexts {
            ...
            let hasBadgeContext =
                (!normalizedAppName.isEmpty && normalizedText.contains(normalizedAppName))
                || normalizedText.contains("notification")
                || normalizedText.contains("unread")
                || normalizedText.contains("badge")
                || normalizedText.contains("new")
