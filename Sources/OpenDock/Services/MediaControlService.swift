...
enum MediaControlService {
    static let didUpdateArtworkNotification = Notification.Name(AppIdentity.mediaArtworkNotificationName)
    ...
    static let pollingQueue = DispatchQueue(
        label: "\(AppIdentity.bundleIdentifier).media-polling",
        qos: .userInitiated
    )
    ...
    private static func artworkCacheDirectory() -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent(AppIdentity.mediaArtworkCacheDirectoryName, isDirectory: true)
