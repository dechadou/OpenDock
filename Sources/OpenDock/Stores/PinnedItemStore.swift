...
public final class PinnedItemStore: ObservableObject {
    ...
    public func load() {
        ...
        do {
            ...
            items = []
            NSLog("\(AppIdentity.displayName): failed to load pinned items: \(error.localizedDescription)")
        }
    }
    ...
    private func save() {
        do {
            ...
            NSLog("\(AppIdentity.displayName): failed to save pinned items: \(error.localizedDescription)")
        }
    }
