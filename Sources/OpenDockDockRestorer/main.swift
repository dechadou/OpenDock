...
guard let parsedArguments = parse(arguments: arguments) else {
    fputs("usage: \(AppIdentity.dockRestorerExecutableName) --watch-pid <pid> --snapshot <path> [--agent-plist <path>]\n", stderr)
    exit(2)
}
...
func runProcess(_ executablePath: String, _ arguments: [String]) {
    ...
    do {
        ...
        fputs("\(AppIdentity.dockRestorerExecutableName): \(error.localizedDescription)\n", stderr)
    }
}
